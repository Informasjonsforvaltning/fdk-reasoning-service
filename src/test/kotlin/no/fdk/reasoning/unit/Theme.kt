...
import no.fdk.reasoning.service.ThemeService
...
class Theme {
    ...
    private fun setupReferenceDataCacheMocks() {
        every { referenceDataCache.los() } returns
            responseReader
                .parseTurtleFile("rdf-data/reference-data/los.ttl")
        every { referenceDataCache.eurovocs() } returns
            responseReader
                .parseTurtleFile("rdf-data/reference-data/eurovocs.ttl")
        every { referenceDataCache.dataThemes() } returns
            responseReader
                .parseTurtleFile("rdf-data/reference-data/data_themes.ttl")
        every { referenceDataCache.mobilityThemes() } returns
            responseReader
    ...
    internal inner class Dataset {
        ...
        fun `test data theme`() {
            ...
            input.add(
                input.getResource(datasetURI),
                DCAT.theme,
                input.createResource("http://publications.europa.eu/resource/authority/data-theme/ENER"),
            )
        ...
        fun `test mobility theme`() {
            ...
            input.add(
                input.getResource(datasetURI),
                ResourceFactory.createProperty("https://w3id.org/mobilitydcat-ap#mobilityTheme"),
                input.createResource("https://w3id.org/mobilitydcat-ap/mobility-theme/traffic-volume"),
            )
