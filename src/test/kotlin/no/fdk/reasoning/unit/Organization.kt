...
import no.fdk.reasoning.service.OrganizationCatalogAdapter
import no.fdk.reasoning.service.OrganizationService
...
class Organization {
    ...
    private fun setupReferenceDataCacheMocks() {
        every { referenceDataCache.organizations() } returns
            responseReader
                .parseTurtleFile("rdf-data/organization-catalog/orgs.ttl")

        every { orgAdapter.orgPathAdapter(any(), any()) } returns "/GENERATED/ORG/PATH"
        every { orgAdapter.downloadOrgData("http://localhost:5050/organizations/972417866") } returns
            responseReader
    ...
    internal inner class Concept {
        ...
        fun `test add triples for concept publisher`() {
            ...
            input.add(
                ...
                DCTerms.publisher,
                input.createResource("https://data.brreg.no/enhetsregisteret/api/enheter/910298062"),
            )
