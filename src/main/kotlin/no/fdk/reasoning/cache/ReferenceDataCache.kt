...
class ReferenceDataCache(
    ...
    fun updateOrganizations() = refresh("organization", uris.orgInternal, ORGANIZATIONS)

    @Scheduled(cron = "0 30 23 * * ?")
    fun updateLOS() = refresh("LOS", uris.los, LOS)

    @Scheduled(cron = "0 15 23 * * ?")
    fun updateEUROVOC() = refresh("EUROVOCS", uris.eurovocs, EUROVOCS)

    @Scheduled(cron = "0 40 23 * * ?")
    fun updateDataThemes() = refresh("data themes", uris.dataThemes, DATA_THEMES)

    @Scheduled(cron = "0 42 23 * * ?")
    fun updateMobilityThemes() = refresh("mobility themes", uris.mobilityThemes, MOBILITY_THEMES)

    @Scheduled(cron = "0 45 23 * * ?")
    fun updateConceptStatuses() = refresh("concept status", uris.conceptStatuses, CONCEPT_STATUSES)

    @Scheduled(cron = "0 50 * * * ?")
    fun updateConceptSubjects() = refresh("concept subjects", uris.conceptSubjects, CONCEPT_SUBJECTS)

    @Scheduled(cron = "0 45 22 * * ?")
    fun updateMediaTypes() = refresh("IANA media types", uris.ianaMediaTypes, MEDIA_TYPES)

    @Scheduled(cron = "0 40 22 * * ?")
    fun updateFileTypes() = refresh("EU file types", uris.fileTypes, FILE_TYPES)

    @Scheduled(cron = "0 35 22 * * ?")
    fun updateLicences() = refresh("licences", uris.licences, LICENCES)
    ...
    fun updateFrequencies() = refresh("frequencies", uris.frequencies, FREQUENCIES)

    @Scheduled(cron = "0 50 21 * * ?")
    fun updateProvenance() = refresh("provenance", uris.provenance, PROVENANCE)

    @Scheduled(cron = "0 45 21 * * ?")
    fun updatePublisherTypes() = refresh("publisher types", uris.publisherTypes, PUBLISHER_TYPES)

    @Scheduled(cron = "0 40 21 * * ?")
    fun updateAdmsStatuses() = refresh("adms statuses", uris.admsStatuses, ADMS_STATUSES)

    @Scheduled(cron = "0 35 21 * * ?")
    fun updateRoleTypes() = refresh("role types", uris.roleTypes, ROLE_TYPES)

    @Scheduled(cron = "0 30 21 * * ?")
    fun updateEvidenceTypes() = refresh("evidence types", uris.evidenceTypes, EVIDENCE_TYPES)

    @Scheduled(cron = "0 25 21 * * ?")
    fun updateChannelTypes() = refresh("channel types", uris.channelTypes, CHANNEL_TYPES)

    @Scheduled(cron = "0 20 21 * * ?")
    fun updateMainActivities() = refresh("main activities", uris.mainActivities, MAIN_ACTIVITIES)

    @Scheduled(cron = "0 15 21 * * ?")
    fun updateWeekDays() = refresh("week days", uris.weekDays, WEEK_DAYS)

    @Scheduled(cron = "0 10 21 * * ?")
    fun updateDatasetTypes() = refresh("dataset types", uris.datasetTypes, DATASET_TYPES)
    ...
    fun updateMobilityConditions() = refresh("mobility conditions", uris.mobilityConditions, MOBILITY_CONDITIONS)

    @Scheduled(cron = "0 50 20 * * ?")
    fun updateHighValueCategories() = refresh("high value categories", uris.highValueCategories, HIGH_VALUE_CATEGORIES)

    @Scheduled(cron = "0 45 20 * * ?")
    fun updateQualityDimensions() = refresh("quality dimensions", uris.qualityDimensions, QUALITY_DIMENSIONS)

    @Scheduled(cron = "0 40 20 * * ?")
    fun updateLegalResourceTypes() = refresh("legal resource types", uris.legalResourceTypes, LEGAL_RESOURCE_TYPES)

    @Scheduled(cron = "0 35 20 * * ?")
    fun updateGeonames() = refresh("geonames", uris.geonames, GEONAMES)

    @Scheduled(cron = "0 30 20 * * ?")
    fun updateEuContinents() = refresh("EU continents", uris.euContinents, EU_CONTINENTS)
