...
@Service
class ReferenceDataService(private val referenceDataCache: ReferenceDataCache) : Reasoner {
    override fun reason(inputModel: Model, catalogType: CatalogType): Model = modelOfContainedReferenceData(
        inputModel,
        catalogType.completeReferenceDataModel(),
    )

    private fun CatalogType.completeReferenceDataModel(): Model = when (this) {
    ...
    private fun conceptReferenceData(): Model = requireAndUnion(
        "Concept statuses" to referenceDataCache.conceptStatuses(),
        "Concept subjects" to referenceDataCache.conceptSubjects(),
    )

    private fun dataServiceReferenceData(): Model = requireAndUnion(
    ...
    private fun informationModelReferenceData(): Model = requireAndUnion(
        "Licences" to referenceDataCache.licences(),
        "Languages" to referenceDataCache.languages(),
        "Locations" to referenceDataCache.locations(),
    )
