...
private fun CatalogType.extendedOrganizationsModel(orgData: Model, catalogData: Model, orgURI: String): Model {
    val organizationPredicates = when (this) {
        CatalogType.PUBLICSERVICES -> listOf(CV.hasCompetentAuthority, CV.ownedBy)
        else -> listOf(DCTerms.publisher)
    }
    val organizationResources = if (this == CatalogType.DATASETS) {
        catalogData.extractOrganizations(organizationPredicates).plus(catalogData.extreactQualifiedAttributionAgents())
    } else catalogData.extractOrganizations(organizationPredicates)
    return orgData.createModelOfOrganizationsWithOrgData(
        organizationURIs = organizationResources
            ...
            .toSet(),
        orgsURI = orgURI
    ).addOrgPathAndNameWhenMissing(organizationResources.toSet(), catalogData, orgData, orgURI)
}

private fun Model.addOrgPathAndNameWhenMissing(organizations: Set<Resource>, catalogData: Model, orgData: Model, orgBaseURI: String): Model {
